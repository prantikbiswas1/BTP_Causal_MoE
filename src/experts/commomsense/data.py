...
COMMONSENSE_OUTPUT = "/home/rupeshk_iitp/Prantik/BTP/BTP_Causal_MoE/data/experts/commonsense/commonsense_train_exploded.jsonl"

def prepare_commonsense_expert_data():
    exploded_dataset = []
    ...
    print("🚀 Exploding CLEAN Commonsense Expert Traces...")
                ...
                if tag == "[COMMONSENSE]":
                    ...
                    if norm_content not in norm_history and "Atomic" not in clean_content:
                        exploded_dataset.append({
                            "id": f"{item['id']}_commonsense_{i}",
                            "instruction": instr,
                            "context_before": " ".join(context_list).strip(),
                            "commonsense_step": current_full_step 
                        })
                
                context_list.append(current_full_step)

    os.makedirs(os.path.dirname(COMMONSENSE_OUTPUT), exist_ok=True)
    with open(COMMONSENSE_OUTPUT, 'w') as f_out:
        for entry in exploded_dataset:
            f_out.write(json.dumps(entry) + "\n")

    print(f"✅ Success! Commonsense path: {COMMONSENSE_OUTPUT}")

if __name__ == "__main__":
    prepare_commonsense_expert_data()